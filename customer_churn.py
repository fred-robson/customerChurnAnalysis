...
def calculate_relative(revenue_list):
	"""
...
def csv_string_to_float(revenue_list_str):
	"""
...
def check_churned_in_tp(customer_revenue, tp, churn_period):
	"""
...
def calculate_time_period_churned(customer_revenue, churn_period):
	"""
	...
	for tp in range(len(customer_revenue)):
		...
		if(started and tp_revenue == 0): 
			if(check_churned_in_tp(customer_revenue, tp, churn_period)):
...
def calculate_churn_rate_absolute(absolute_time_period, churn_period):
	pass

def calculate_churn_tp_dict(relative_revenue, churn_period):
	"""
	...
	for customer in relative_revenue:
		...
		time_period_churned = calculate_time_period_churned(customer_revenue,churn_period)
...
def read_customer_data(filename):
	...
	with open(filename) as csvfile:
		...
		for row in fileReader:
			...
			revenue = csv_string_to_float(row[1:])
			absolute_time_period[customer] = revenue
			relative_time_period[customer] = calculate_relative(revenue)
	return (absolute_time_period, relative_time_period)

churn_period = 2 #Churn Period is the number of months a customer has to be inactive in order to be counted as churned
data = read_customer_data("customer_churn_test.csv")
absolute = data[0]
relative = data[1]
relative_churned_time = calculate_churn_rate_relative(relative,churn_period)
print relative_churned_time["Lexus Santa Monica"]
